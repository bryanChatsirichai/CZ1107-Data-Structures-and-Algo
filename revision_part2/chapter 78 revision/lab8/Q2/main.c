...
void myfunc(Graph_DFS g, int v,int visited[]);
...
void myfunc(Graph_DFS g, int v,int visited[])
{
    ...
    for(i = 0;i<g.V;i++)
    {
        if(g.matrix[cur][i] == 1 && visited[i] == -1 )
        {
            ...
            myfunc(g,i+1,visited);
        }
    }
    printf("%d ",cur + 1); //after checking all neighbours or none cur node has to backtrack
