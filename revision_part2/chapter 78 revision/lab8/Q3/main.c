...
int nQueens(int** board, int BSize, int col)
{
    ...
    if(col == BSize)
    {
        ///if col same as bsize means mange but n number of queens
        count++;
        printf("count is %d\n",count);
    ...
    {    /// check row by row in the (col)^th column
        ///row control the up/down (y-axis)
        ///col control the left/right (x-axis)
        int row;
        for(row=0;row<BSize;row++)
        {
            if(isSafe(board,BSize,row,col))
            {
                ///current board is acceptable
                 board[row][col] = 1; //add queen to this position

                 nQueens(board,BSize,col+1);// recursive the next column placement
