#include <stdio.h>
#include <stdlib.h>
#define SIZE 3 //assume 3 x 3 array
void swap2Rows(int ar[][SIZE],int r1,int r2); //int (*matrix)[SIZE]
...
void swap2Rows(int ar[][SIZE],int r1,int r2){
    int i,j;
    int temp;
    for(j=0;j<SIZE;j++) //[i][j] [j] is the moving i is the row swap
    {   //draw out matrix in [i][j] to visualise
            temp = ar[r1][j];
            ar[r1][j] = ar[r2][j];
            ar[r2][j] = temp;
    }
...
void swap2Cols(int ar[][SIZE],int c1,int c2){
    int i,j;
    int temp;
    for(i = 0;i<SIZE;i++) //[i][j] [i] is the moving j is the column swap
    {
        temp = ar[i][c1];
        ar[i][c1] = ar[i][c2];
        ar[i][c2] = temp;
