...
int main()
{
    ...
    int maxMatch;
    int i;
    int p_num,m_num,s_num,count,source,sink;
	...
	Graph g;
	g.V = Std + Prj + Mtr + 2; //including s and t
    ...
    for(i = 0;i < Std;i++)
    {
        ...
        {
            //student adjacent to mentor
            //printf("Enter mentor number\n",m_num);
            scanf("%d",&m_num);
            m_num = m_num + Prj + Std ; //offset the index
            insertAdjVertex(&g.list[s_num],m_num);
        }
...
int bfs(Graph g,int **r_graph,int source,int target,int parent_tracker[])
{
    ...
    int i,current_node,neighbour_node;
...
int fordfulerson(Graph g,int source,int target)
{
    ...
    while(bfs(g,r_graph,source,target,parent_tracker))
    {
        ...
        while( v != source)
        {
            ...
            also need to decrease the flow along each residual edge (doted egde going backward of agumented path)
            ...
            */
            u = parent_tracker[v];
            r_graph[u][v] = r_graph[u][v] - bottle_neck;
            r_graph[v][u] = r_graph[v][u] + bottle_neck;
            v = parent_tracker[v];
        }
        maxflow = maxflow + bottle_neck;
