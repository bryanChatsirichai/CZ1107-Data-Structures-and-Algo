...
void sumToC(LinkedList* ll, int C, ArrayList* al,int start,int arr[],int length)
{
    ...
    else
    {
        ...
        for(i;i<length;i++)
        {
            ...
            sumToC(ll,remainder,al,i+1,arr,length); // i + 1 as to check all other nodes possible excluding current
            removeNode(ll,ll->size - 1); //remove last node in LL as backtrack from a fail condition
            if(C - arr[i + 1] < 0)
            {
