...
{   /*start from a vertex and explore it as far as possible before backing up */
    ...
    ListNode *temp = NULL;
    node = start;
    push(&s,node); //suspend exploration by pushing to stack and explore as deeply as possible
    visited[node] = 1; //when visited set to 1 and push to stack
    while(!isEmptyStack(s)) //continue until stack is empty
    {
        top = peek(s);
        temp = g.list[top];
        while(temp != NULL)
        {   //check the vertex has any neighbours
            neighbour = temp->vertex - 1; //due to the indexing calculation
            if(visited[neighbour] == -1) //have neighbours and haven visited it before
            {
                visited[neighbour] = 1; //visit the neighbour
                push(&s,neighbour); //suspend exploration by pushing to stack and explore as deeply as possible
                break; //do not check by level want to go as deeply as possible
            }
            temp = temp->next; //if current vertex neighbours visited check if any others haven
        }
        if(temp == NULL) //visited all adjacent nodes or no adjacent nodes
        {
            pop(&s); //backtracking
        }
    }
