...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	ListNode *q,*p,*temp,*front_pre,*back_pre,*front_cur,*back_cur;
	front_cur = resultFrontList;
	int i;
	p = ll->head; //main Linklist
	//use this to control loop
	//Suppose be even in the case odd size first half size +1 more than second
	int first_half;
	int second_half;
	//printf("ll size is %d\n",ll->head->item);
	...
	if(ll->size % 2 == 0)
    {
        first_half = ll->size / 2;
        second_half  = ll->size / 2;
    }
    else if(ll->size % 2 == 1)
    {
        first_half = ( (ll->size -1) / 2) + 1;
        second_half  = ( (ll->size -1) / 2);
    }
    //printf("first half size is %d\n",first_half);
    for(i = 0;i<first_half;i++)// do front list first
    {
        if(resultFrontList->head == NULL) //first time null
        {   //printf("option 1\n");
            resultFrontList->head = p;
            p = p->next;
            front_cur = resultFrontList->head;
            front_cur->next = NULL;
    ...
    for(i = 0;i<second_half;i++)// do front list first
    {
        if(resultBackList->head == NULL) //first time null
        {   //printf("option 1\n");
            resultBackList->head = p;
            p = p->next;
            back_cur = resultBackList->head;
            back_cur->next = NULL;
