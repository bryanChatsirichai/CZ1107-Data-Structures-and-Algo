...
BTNode *searchNode(BTNode *node, int key)
{
    ...
    {   printf("checking node %d\n",node->item);
        if(node->item ==  key)
            return node; //after the root node if key found will return to the temp upwards the recursive call
        else
        {
            temp = searchNode(node->left,key); //traverse left side
            if(temp != NULL)
            {   //if found return the node upwards the recursive call
                return temp;
            }
            else if(temp == NULL) //pre order left traverse no find try right side
            {
                //if right side have then return node address
                //if no have temp will still return null
