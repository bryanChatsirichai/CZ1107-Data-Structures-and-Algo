...
void preOrderIterative(BSTNode *root)
{
    ...
    else
    {
        ...
        while(p!= NULL || !isEmpty(&s))
        {
            if(p != NULL)
            {
                //show node then left right
                printf("%d ",p->item);
                push(&s,p);// after printing push to stack
                p = p->left; //traverse left sub tree
            }
            else if(p == NULL)
            {
                //reach leaf node and its left  is empty;
                p = pop(&s);
                p = p->right; //check the right side if left null
            }
        }
    }
}
