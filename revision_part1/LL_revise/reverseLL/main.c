...
void reverseLL(LinkedList *ll){
    ...
    else
    {
        ...
        while(c != NULL)
        {
            //sliding all 3 pointers
            //note head and tail must swap
            a = b;
            b = c;
            c = c->next;

            //reconnecting them in reverse
            b->next = a;
        }
        //need set new head at the last node of the original LL
        //Since c is null to exit loop b will be at the last node
        //note head and tail must swap
        //only change the linking so head still at forst node
       //swap tail and head
       temp = ll->tail;
       ll->tail = ll->head; //reverse tail will point at the previous forst node
       ll->head = temp; //head will now point at the last node
    }
}
