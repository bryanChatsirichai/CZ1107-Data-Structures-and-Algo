...
{
    ListNode *cur;
	int total_count,front_count,back_count,indicies_1,indicies_2;
    ...
    else
    {
        ...
        indicies_2 = 0;
        total_count = ll->size;
        front_count =  ((total_count - 1)/2) + 1 ;
        back_count = ((total_count -1)/ 2);

        //printf("front indices %d  and back %d\n",front_count,back_count);
        while(cur != NULL)
        {
            printf("cur item %d\n",cur->item);
            if(indicies_1 < front_count)
            //for the first half if odd will be +1
            {
                insertNode(resultFrontList,indicies_1,cur->item);
                indicies_1++; //control the index position
                removeNode(ll,0);//after inserting remove the original at index 0
            }
            else
            {   //after on top controlling the first half
                //the reaming of the original will be inserted into backLL
                insertNode(resultBackList,indicies_2,cur->item);
                indicies_2++; //control the index position
                removeNode(ll,0);//after inserting remove the original at index 0
            }
            cur = cur->next; //iterating through the original LL

        }
        printf("original LL size is %d\n",ll->size);
        free(ll->head);
