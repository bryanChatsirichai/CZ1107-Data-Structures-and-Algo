...
void appendLL(LinkedList *ll_a , LinkedList *ll_b)
{
    ...
    if(ll_a->head == NULL && ll_b->head == NULL)
        return -1;
    else if(ll_a != NULL && ll_b == NULL)
        return 0;
    else if(ll_a->head == NULL && ll_b->head != NULL)
    {
        ll_a->head = ll_b->head;
        ll_a->size = ll_a->size + ll_b->size;
        ll_b->head = NULL;
        ll_b->size = 0;
        printf("After A size is %d B size is %d\n",ll_a->size,ll_b->size);
        return 0;
    }
    else
    {
