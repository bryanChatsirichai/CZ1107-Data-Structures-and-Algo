...
//if root address change we need return it back
{
    ...
    else// if ((*node)->item ==value) found the node!
    {
        ...
        {   printf("case 1\n");
            temp = node;//deallocate memory
            free(temp);
            //set node as null so the parent of this node points to Null when this is return
            node = NULL;
        ...
        {
            /*find min in right tree/subtree
            copy the value to targeted node
            delete duplicate from right subtree; */
            printf("case 3\n");
            //find the minimum in the node to be deleted right subtree
            p = findMin(node->right); //this will take the node place
            //transforming case 3 to case 1 or 2
            //let the node to be deleted also be the min that take over it
            //delete the node that it became which should be on its right child
            //delete
            node->item = p->item;
...
{   /*if a tree or subtree has a min value it wont have a left child
    ...
    if(p->left != NULL)
    {
        ...
        return p; //when p has no left child means smallest already
}
