...
{   //return null upwards if hit leaf node left and right nothing
    //if found will return upwards and not continue to check left side
    BTNode *p;
    if(node == NULL)
        return NULL;
    else
    {
        ...
        {
            p = searchNode(node->left,key);
            if(p != NULL)
                return p;
            else if(p == NULL)
            {   //check left subtree if right subtree fail
                p = searchNode(node->right,key);
                return p;
            }
        }
    }
