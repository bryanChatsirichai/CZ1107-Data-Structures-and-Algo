...
int isStackPairwiseConsecutive(Stack *s)
{
    ...
    if(s->ll.size <= 0 || s->ll.size % 2 == 1)
        return 0 ; //empty stack or odd size cant form pair
    else
    {
        while(!isEmptyStack(s))
        {
            k = pop(s);
            kplus1 = pop(s);
            printf("k is %d and kplus1 is %d\n",k,kplus1);
            check = k - kplus1;
            if(check < 0)
            {   //convert to positive is negative
                //take the abs value
                check = check * - 1;
            }
            if(check != 1)
        ...
        return 1; //go through whole stack then pass
    }
    return 0; //default return fail
}
