...
void reverseStack(Stack *s)
{
	Queue q; //local to the function
	q.ll.head = NULL;
	q.ll.tail = NULL;
	q.ll.size = 0;
	int item;
    //pop the stack and enqueue it
	while(!isEmptyStack(s))
    {
        item = pop(s);
        enqueue(&q,item);
    }
    //dequeue the queue and push the item to the stack
    //this will reverse the item of the original stack
